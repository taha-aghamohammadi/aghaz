...
export function SiteHeader() {
  ...
  function scrollToDesks(e: React.MouseEvent) {
    if (!isLanding) return;
    e.preventDefault();
    scrollToLiveMap();
  }
          ...
          {links.map((l) =>
            l.href.startsWith("#") ? (
              <a
                key={l.href}
                href={l.href}
                onClick={l.href === "#desks" ? scrollToDesks : undefined}
      ...
      {open && (
        <div className="border-t border-hairline bg-background md:hidden">
          <div className="flex flex-col gap-1 px-4 py-4 sm:px-6">
            {links.map((l) =>
              l.href.startsWith("#") ? (
                <a
                  ...
                  href={l.href}
                  onClick={(e) => {
                    if (l.href === "#desks") scrollToDesks(e);
                    setOpen(false);
                  }}
            ...
            {isLanding ? (
              <Button className="mt-2 rounded-full" onClick={() => { setOpen(false); scrollToLiveMap(); }}>
                رزرو میز
              </Button>
            ) : (
              <Button
